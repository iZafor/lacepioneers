...
export const getShoesBy = query({
    ...
    handler: async (ctx, { field, value, take, distinct, select }) => {
        ...
        const filterQuery =
            field === undefined || value === undefined
                ? distinctQuery
                : distinctQuery.filterWith(async (q) => q[field] === value);

        const selectQuery =
            select === undefined
                ? filterQuery
                : filterQuery.map(async (q) => {
                      const res: { [key: string]: typeof value } = {};
...
export const updateShoeData = mutation({
    args: {
        id: v.id("shoes"),
        values: v.array(v.object({ field: v.string(), value })),
    },
